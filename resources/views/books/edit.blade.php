...
@section('content')
    <div class="bg-white p-6 rounded-lg shadow max-w-xl mx-auto mt-6">
        <form action="{{ route('books.update', $book) }}" method="POST" enctype="multipart/form-data" class="text-sm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="4"
                    class="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none @error('description') border-red-500 @enderror">{{ old('description', $book->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if ($book->thumbnail)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $book->thumbnail) }}" alt="Thumbnail" class="h-24 rounded shadow">
                    </div>
                @endif
